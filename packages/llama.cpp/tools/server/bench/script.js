...
const n_prompt = __ENV.SERVER_BENCH_N_PROMPTS
  ? parseInt(__ENV.SERVER_BENCH_N_PROMPTS)
  : (600 / 10) * 8;
...
export function setup() {
  console.info(
    `Benchmark config: server_url=${server_url} n_prompt=${n_prompt} model=${model} dataset_path=${dataset_path} max_tokens=${max_tokens}`
  );
}
...
const data = new SharedArray('conversations', function () {
  ...
  return (
    JSON.parse(open(dataset_path))
      ...
      .filter(
        (conv) =>
          conv.n_prompt_tokens <= n_prompt_tokens &&
          conv.n_prompt_tokens + conv.n_completion_tokens <= n_ctx_slot
      )
...
const llamacpp_completion_tokens_total_counter = new Counter(
...
export default function () {
  ...
  const res = sse.open(`${server_url}/chat/completions`, params, function (client) {
    client.on('event', function (event) {
      if (promptEvalEndTime == null) {
        ...
        llamacpp_emit_first_token_second.add((promptEvalEndTime - startTime) / 1e3);
      }
  ...
  if (promptEvalTime > 0) {
    llamacpp_prompt_processing_second.add((prompt_tokens / (promptEvalEndTime - startTime)) * 1e3);
  }

  const completion_time = endTime - promptEvalEndTime;
  if (completions_tokens > 0 && completion_time > 0) {
    llamacpp_tokens_second.add((completions_tokens / completion_time) * 1e3);
  }
