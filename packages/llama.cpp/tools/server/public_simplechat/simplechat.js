...
class SimpleChat {
  ...
  recent_chat(iRecentUserMsgCnt) {
    ...
    if (iRecentUserMsgCnt == 0) {
      console.warn(
        'WARN:SimpleChat:SC:RecentChat:iRecentUsermsgCnt of 0 means no user message/query sent'
      );
    }
    /** @type{ChatMessages} */
    let rchat = [];
    ...
    if (this.xchat.length == 0) {
      ...
      if (sysPrompt.length > 0) {
        if (this.xchat[0].role !== Roles.System) {
          console.error(
            `ERRR:SimpleChat:SC:${msgTag}:You need to specify system prompt before any user query, ignoring...`
          );
        } else {
          if (this.xchat[0].content !== sysPrompt) {
            console.error(
              `ERRR:SimpleChat:SC:${msgTag}:You cant change system prompt, mid way through, ignoring...`
            );
          }
        }
      }
  ...
  async handle_response_oneshot(resp, apiEP) {
    ...
    console.debug(
      `DBUG:SimpleChat:SC:${this.chatId}:HandleUserSubmit:RespBody:${JSON.stringify(respBody)}`
    );
...
class MultiChatUI {
  ...
  async handle_user_submit(chatId, apiEP) {
    ...
    if (chatId == this.curChatId) {
      ...
      console.debug(
        `DBUG:SimpleChat:MCUI:HandleUserSubmit:ChatId has changed:[${chatId}] [${this.curChatId}]`
      );
    }
    this.ui_reset_userinput();
  }
  ...
  show_sessions(elDiv = undefined) {
    ...
    let btnNew = ui.el_create_button('New CHAT', (ev) => {
      if (this.elInUser.disabled) {
        console.error(
          `ERRR:SimpleChat:MCUI:NewChat:Current session [${this.curChatId}] awaiting response, ignoring request...`
        );
        alert(
      ...
      let chatIdGot = prompt(
        'INFO:SimpleChat\nMCUI:NewChat\nEnter id for new chat session',
        chatId
      );
      if (!chatIdGot) {
  ...
  create_session_btn(elDiv, cid) {
    let btn = ui.el_create_button(cid, (ev) => {
      ...
      if (this.elInUser.disabled) {
        console.error(
          `ERRR:SimpleChat:MCUI:SessionClick:${target.id}:Current session [${this.curChatId}] awaiting response, ignoring switch...`
        );
        alert(
...
class Me {
  ...
  show_info(elDiv, bAll = false) {
    ...
    if (bAll) {
      ...
      ui.el_create_append_p(
        `bCompletionInsertStandardRolePrefix:${this.bCompletionInsertStandardRolePrefix}`,
        elDiv
      );
    }

    ui.el_create_append_p(
      `apiRequestOptions:${JSON.stringify(this.apiRequestOptions, null, ' - ')}`,
      elDiv
    );
  ...
  show_settings_apirequestoptions(elDiv) {
    ...
    for (const k in this.apiRequestOptions) {
      ...
      if (type == 'string' || type == 'number') {
        let inp = ui.el_creatediv_input(
          `Set${k}`,
          k,
          typeDict[type],
          this.apiRequestOptions[k],
          (val) => {
            if (type == 'number') {
              val = Number(val);
            }
        ...
        let bbtn = ui.el_creatediv_boolbutton(
          `Set{k}`,
          k,
  ...
  show_settings(elDiv) {
    ...
    let bb = ui.el_creatediv_boolbutton(
      'SetStream',
      'Stream',
      { true: '[+] yes stream', false: '[-] do oneshot' },
      this.bStream,
      (val) => {
        this.bStream = val;
      }
    ...
    bb = ui.el_creatediv_boolbutton(
      'SetTrimGarbage',
      'TrimGarbage',
      { true: '[+] yes trim', false: '[-] dont trim' },
      this.bTrimGarbage,
      (val) => {
    ...
    sel = ui.el_creatediv_select(
      'SetChatHistoryInCtxt',
      'ChatHistoryInCtxt',
      this.sRecentUserMsgCnt,
      this.iRecentUserMsgCnt,
      (val) => {
    ...
    bb = ui.el_creatediv_boolbutton(
      'SetCompletionFreshChatAlways',
      'CompletionFreshChatAlways',
      { true: '[+] yes fresh', false: '[-] no, with history' },
      this.bCompletionFreshChatAlways,
      (val) => {
    ...
    bb = ui.el_creatediv_boolbutton(
      'SetCompletionInsertStandardRolePrefix',
      'CompletionInsertStandardRolePrefix',
      { true: '[+] yes insert', false: '[-] dont insert' },
      this.bCompletionInsertStandardRolePrefix,
      (val) => {
