...
class APITester {
  ...
  async runAllTests() {
    ...
    console.log(
      chalk.gray(`Timeout: ${TEST_CONFIG.timeout}ms | Retries: ${TEST_CONFIG.retries}\n`)
    );
  ...
  async testInsertionOrderEndpoints() {
    ...
    await this.runTest('List Insertion Orders', async () => {
      ...
      this.assert(
        response.some((io) => io.id === createdIOId),
        'Should contain created IO'
      );
    ...
    if (createdIOId) {
      await this.runTest('Approve Insertion Order', async () => {
        const response = await this.makeRequest(
          'PUT',
          `/api/insertion-orders/${createdIOId}/approve`,
          {
  ...
  async testDailySpendEndpoints() {
    ...
    await this.runTest('Batch Record Daily Spend', async () => {
      ...
      const response = await this.makeRequest('POST', '/api/daily-spend/batch', {
        records: batchData,
      });
    ...
    await this.runTest('Query Daily Spend Data', async () => {
      const response = await this.makeRequest(
        'GET',
        '/api/daily-spend?start_date=2025-01-01&end_date=2025-01-31'
      );
  ...
  async testAnalyticsEndpoints() {
    ...
    await this.runTest('Performance Analytics', async () => {
      const response = await this.makeRequest(
        'GET',
        '/api/analytics/performance?start_date=2025-01-01&end_date=2025-01-31'
      );
      ...
      this.assert(
        response.generated_by.includes('flow_nexus') || response.generated_by.includes('ai'),
        'Should be generated by Flow Nexus AI'
      );

      console.log(chalk.gray(`      Generated by: ${response.generated_by}`));
      console.log(
        chalk.gray(`      Period: ${response.period.start_date} to ${response.period.end_date}`)
      );
    ...
    await this.runTest('Pacing Analysis', async () => {
      ...
      console.log(
        chalk.gray(`      Pacing status: ${response.pacing_analysis.status || 'analyzed'}`)
      );
  ...
  async testOptimizationEndpoints() {
    ...
    await this.runTest('AI Budget Optimization', async () => {
      ...
      console.log(
        chalk.gray(
          `      Expected ROAS increase: ${response.expected_impact.roas_increase || 'N/A'}`
        )
      );
    ...
    await this.runTest('Get AI Recommendations', async () => {
      const response = await this.makeRequest(
        'GET',
        '/api/optimize/recommendations?insertion_order_id=1'
      );
  ...
  displayResults() {
    ...
    console.log(
      chalk.white(`Success Rate: ${((this.results.passed / this.results.total) * 100).toFixed(1)}%`)
    );
