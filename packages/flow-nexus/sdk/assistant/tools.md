...
When tool execution is enabled, Queen Seraphina can access the full range of
Flow Nexus capabilities to assist with your projects.
...
const response = await mcp_flow_nexus_seraphina_chat({
  message:
    'Create a development swarm for a React application with testing and deployment',
  enable_tools: true,
});
...
const mlRequest = await mcp_flow_nexus_seraphina_chat({
  message:
    'Train a classification model for email spam detection using our dataset',
  enable_tools: true,
});
...
const devEnv = await mcp_flow_nexus_seraphina_chat({
  message:
    'Set up a Python environment for data science work with common libraries',
  enable_tools: true,
});
...
const complexProject = await mcp_flow_nexus_seraphina_chat({
  message:
    'Bootstrap a full-stack application with AI features, testing, and deployment pipeline',
  enable_tools: true,
});
...
const optimizedSetup = await mcp_flow_nexus_seraphina_chat({
  message:
    'Optimize my current swarm setup for better performance and cost efficiency',
  enable_tools: true,
});
...
const mlPipeline = await mcp_flow_nexus_seraphina_chat({
  message:
    'Create a complete machine learning pipeline for predicting customer churn',
  enable_tools: true,
});
...
Tool-enabled conversations with Seraphina transform natural language requests
into fully implemented, production-ready solutions.
