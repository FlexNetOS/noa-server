...
export const HooksEditor: React.FC<HooksEditorProps> = ({
  ...
  const renderMatcher = (event: HookEvent, matcher: EditableHookMatcher) => (
    <Card key={matcher.id} className="space-y-4 p-4">
  ...
  const renderDirectCommand = (event: HookEvent, command: EditableHookCommand) => (
    <Card key={command.id} className="space-y-2 p-4">
      <div className="flex items-start gap-2">
        <div className="flex-1 space-y-2">
          <Textarea
            placeholder="Enter shell command..."
    ...
    <div className={cn('space-y-6', className)}>
      ...
      {isLoading && (
        <div className="flex items-center justify-center p-8">
          ...
          <span className="text-muted-foreground text-sm">Loading hooks configuration...</span>
        </div>
      )}

      {/* Error State */}
      {loadError && !isLoading && (
        <div className="border-destructive/50 bg-destructive/10 text-destructive flex items-center gap-2 rounded-lg border p-3 text-xs">
      ...
      {!isLoading && (
        <>
          ...
          {validationErrors.length > 0 && (
            <div className="space-y-1 rounded-md bg-red-500/10 p-3">
            ...
            {(Object.keys(EVENT_INFO) as HookEvent[]).map((event) => {
              ...
              return (
                <TabsContent key={event} value={event} className="space-y-4">
                  <div className="space-y-2">
                    <p className="text-muted-foreground text-sm">{EVENT_INFO[event].description}</p>
                  </div>
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      {matcherEvents.includes(template.event as any) && template.matcher && (
                        <p className="bg-muted inline-block rounded px-2 py-1 font-mono text-xs">
                          Matcher: {template.matcher}
                        </p>
                      )}
                    </div>
