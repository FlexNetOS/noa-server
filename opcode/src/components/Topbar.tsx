 ...
 * />
 */
export const Topbar: React.FC<TopbarProps> = ({ onSettingsClick, className }) => {
  ...
  const StatusIndicator = () => {
    if (checking) {
      return (
        <div className="flex items-center space-x-2 text-xs">
          <Circle className="text-muted-foreground h-3 w-3 animate-pulse" />
    ...
    const statusContent = (
      <Button
        variant="ghost"
        size="sm"
        className="hover:bg-accent h-auto px-2 py-1"
          ...
          content={
            <div className="max-w-xs space-y-3">
              ...
              <div className="bg-muted rounded-md p-3">
                <pre className="font-mono text-xs whitespace-pre-wrap">{versionStatus.output}</pre>
              </div>
              ...
              <a
                ...
                className="text-primary flex items-center space-x-1 text-xs hover:underline"
              >
  ...
  return (
    <motion.div
      ...
      className={cn(
        'border-border bg-background/95 supports-[backdrop-filter]:bg-background/60 flex items-center justify-between border-b px-4 py-3 backdrop-blur',
        className
      )}
    >
      {/* Status Indicator */}
