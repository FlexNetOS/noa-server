...
import { Calendar, Filter, Loader2, Briefcase, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    return stats.by_model.slice(0, 3).map((model) => (
      <div key={model.model} className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-caption text-muted-foreground">{model.session_count} sessions</span>
        </div>
        <span className="text-body-small font-medium">{formatCurrency(model.total_cost)}</span>
      </div>
    ));
  ...
  const topProjects = useMemo(() => {
    ...
    return stats.by_project.slice(0, 3).map((project) => (
      <div key={project.project_path} className="flex items-center justify-between">
        ...
        <span className="text-body-small font-medium">{formatCurrency(project.total_cost)}</span>
      </div>
    ));
      ...
      <div className="mx-auto flex h-full max-w-6xl flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="bg-destructive/10 border-destructive/50 text-body-small text-destructive mb-4 rounded-lg border p-3">
              {error}
                ...
                <TabsContent value="overview" className="mt-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <Card className="p-6">
                      ...
                      <div className="space-y-3">{mostUsedModels}</div>
                    </Card>

                    <Card className="p-6">
                      <h3 className="text-label mb-4">Top Projects</h3>
                      <div className="space-y-3">{topProjects}</div>
                    </Card>
                  </div>
                </TabsContent>
                                ...
                                {totalPages > 1 && (
                                  <div className="flex items-center justify-between pt-4">
                                    <span className="text-muted-foreground text-xs">
                                      ...
                                      {Math.min(endIndex, stats.by_project.length)} of{' '}
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <Button
                                        ...
                                        size="sm"
                                        onClick={() =>
                                          setProjectsPage((prev) => Math.min(totalPages, prev + 1))
                                        }
                              ...
                              return (
                                <>
                                  {paginatedSessions.map((session, index) => (
                                    <div
                                      key={`${session.project_path}-${session.project_name}-${startIndex + index}`}
                                      className="border-border flex items-center justify-between border-b py-2 last:border-0"
                                    >
                                      <div className="flex flex-col">
                                        <div className="flex items-center space-x-2">
                                  ...
                                  {totalPages > 1 && (
                                    <div className="flex items-center justify-between pt-4">
                                      <span className="text-muted-foreground text-xs">
                                        Showing {startIndex + 1}-
                                        {Math.min(endIndex, sessionStats.length)} of{' '}
                                        {sessionStats.length}
                                      </span>
                                      <div className="flex items-center gap-2">
                                        <Button
                              ...
                              {timelineChartData.bars.map((day) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    {/* Bar */}
                                    <div
                                      className="bg-primary w-full cursor-pointer rounded-t transition-opacity hover:opacity-80"
                                      style={{ height: `${day.heightPercent}%` }}
                                    />

                                    {/* X-axis label – absolutely positioned below the bar */}
                                    <div className="text-muted-foreground pointer-events-none absolute top-full left-1/2 mt-2 -translate-x-1/2 text-xs whitespace-nowrap">
