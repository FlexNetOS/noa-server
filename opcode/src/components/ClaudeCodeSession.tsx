...
import { Copy, ChevronDown, GitBranch, ChevronUp, X, Hash, Wrench } from 'lucide-react';
...
export const ClaudeCodeSession: React.FC<ClaudeCodeSessionProps> = ({
        ...
        const processComplete = async (success: boolean) => {
          ...
          if (effectiveSession && claudeSessionId) {
            const sessionStartTimeValue =
              messages.length > 0 ? messages[0].timestamp || Date.now() : Date.now();
        ...
        const lastModel =
          sessionMetrics.current.modelChanges.length > 0
            ...
            : sessionMetrics.current.wasResumed
              ? 'sonnet'
              : model; // Default to sonnet if resumed
  ...
  const messagesList = (
    ...
    >
      <div
        className="relative mx-auto w-full max-w-6xl px-4 pt-8 pb-4"
        style={{
      ...
      {isLoading && (
        <motion.div
          ...
          className="mb-20 flex items-center justify-center py-4"
        >
      ...
      {error && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.15 }}
          className="border-destructive/50 bg-destructive/10 text-destructive mx-auto mb-20 w-full max-w-6xl rounded-lg border p-4 text-sm"
        >
          {error}
        </motion.div>
      )}
        ...
        <div className="flex h-full w-full flex-col">
          ...
          <ErrorBoundary>
            ...
            <AnimatePresence>
              {queuedPrompts.length > 0 && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 20 }}
                  className="fixed bottom-24 left-1/2 z-30 w-full max-w-3xl -translate-x-1/2 px-4"
                >
                  <div className="bg-background/95 space-y-2 rounded-lg border p-3 shadow-lg backdrop-blur-md">
                    <div className="flex items-center justify-between">
                      <div className="text-muted-foreground mb-1 text-xs font-medium">
                        Queued Prompts ({queuedPrompts.length})
                      </div>
                      <TooltipSimple
                        content={queuedPromptsCollapsed ? 'Expand queue' : 'Collapse queue'}
                        side="top"
                      >
            ...
            {displayableMessages.length > 5 && (
              ...
              >
                <div className="bg-background/95 flex items-center overflow-hidden rounded-full border shadow-lg backdrop-blur-md">
                  <TooltipSimple content="Scroll to top" side="top">
                    <motion.div whileTap={{ scale: 0.97 }} transition={{ duration: 0.15 }}>
                      <Button
                        ...
                        className="hover:bg-accent rounded-none px-3 py-2"
                      >
                    ...
                    {effectiveSession && (
                      <TooltipSimple content="Session Timeline" side="top">
                        <motion.div whileTap={{ scale: 0.97 }} transition={{ duration: 0.15 }}>
                          ...
                          >
                            <GitBranch
                              className={cn('h-3.5 w-3.5', showTimeline && 'text-primary')}
                            />
                          </Button>
                        </motion.div>
                      </TooltipSimple>
            ...
            {totalTokens > 0 && (
              <div className="pointer-events-none fixed right-0 bottom-0 left-0 z-30">
                <div className="mx-auto max-w-6xl">
                  <div className="flex justify-end px-4 pb-2">
                    <motion.div
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                      exit={{ opacity: 0, scale: 0.8 }}
                      className="bg-background/95 pointer-events-auto rounded-full border px-3 py-1 shadow-lg backdrop-blur-md"
                    >
          ...
          <AnimatePresence>
            {showTimeline && effectiveSession && (
              <motion.div
                ...
                className="bg-background border-border fixed top-0 right-0 z-30 h-full w-full overflow-hidden border-l shadow-xl sm:w-96"
              >
                <div className="flex h-full flex-col">
                  {/* Timeline Header */}
