...
**Relationship to AGENT policy:** `AGENT.md` at the `rtt/` root defines the single
agent/provider policy and Truth Gate requirements for this repository. The background
agents described here must operate within that policy and in alignment with
`rtt-prd.md` and the rest of `docs/`.
...
```json
{
  ...
  "findings": {
    "critical": [],
    "warnings": ["File viewfs/README.md exists, will merge"],
    "info": [
      "25 files extracted successfully",
      "Directory structure matches specification"
    ]
  },
  "recommendations": ["Review merged README.md for conflicts"]
}
```
