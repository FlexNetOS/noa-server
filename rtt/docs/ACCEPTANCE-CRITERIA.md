...
### 11. Feature Flags Guard Non-Owned Integrations

**Criterion:** All non-owned integrations are protected by feature flags with kill-switch and A/B semantics, per `rtt-prd.md` functional requirement 11.

**Validation (procedural):**

- Produce a current list of non-owned integrations (external SaaS APIs, third-party LLMs, cross-team services, etc.).
- For each integration, identify the controlling flag(s) in your feature-flag system, configuration, or environment.
- In a non-production environment, flip the flag **off** while running a minimal RTT smoke test that exercises the integration, then flip it **on** again:
  - When the flag is **off**, RTT should stop routing traffic to that integration and fail gracefully.
  - When the flag is **on**, RTT should resume normal operation without redeploying.
- Record flag locations and rollback procedures in your runbook and keep them in sync with `docs/OPERATIONS.md`.

> The exact mechanism (OpenFeature, LaunchDarkly, in-house flags, env vars) is implementation-specific. The acceptance check is about **observable behavior**: operators must be able to disable or switch variants for any non-owned integration without code changes.

**Success:** ✅ Every non-owned integration has a documented, tested feature flag that can be toggled to disable or switch variants without redeploying RTT.
