#!/usr/bin/env python3
import json, sys, hashlib, glob, os, time
...
Route = Dict[str, str]
RoutesDoc = Dict[str, List[Route]]

def canon(obj: Any) -> bytes:
    return json.dumps(obj, ensure_ascii=False, sort_keys=True, separators=(',', ':')).encode('utf-8')

def saddr_from_agent(ag: Dict[str, Any]) -> str:
    ver = ag.get("version","1.0.0")
    return ag.get("rtt_saddr", f"rtt://agent/api/{ag['id']}@{ver}")

def load_manifests(mdir: str) -> Dict[str, Dict[str, Any]]:
...
def autowire(routes: RoutesDoc, mcp_provider: str, skills_dir: str, agents_dir: str, manifests_dir: str) -> RoutesDoc:
    # Simple strategy: connect agent.id to MCP tool with same name.
    idx_agents: Dict[str, Dict[str, Any]] = {}
    ...
    for t in tools:
        caps: Set[str] = set(t.get("capabilities", []))
        if skill_caps & caps and t.get("name") not in idx_agents:
            ...
            for a in idx_agents.values():
                ...
                if skill_caps & acaps:
                    ...
                    break
    # merge unique
    existing: Set[Tuple[str, str]] = set((r["from"],r["to"]) for r in routes.get("routes",[]))
...
def invariant_gate(routes: RoutesDoc, manifests_dir: str) -> List[Dict[str, Any]]:
...
def main():
    ...
    pid = "sha256-" + hashlib.sha256(payload_bytes).hexdigest()
    plan["plan_id"] = pid
    # Sign
    priv = (ROOT / ".rtt" / "registry" / "keys" / "private" / f"{key_id}.priv").read_text().strip()
    sig = sign_msg(priv, payload_bytes)
    ...
    (ROOT / "plans" / "latest.plan.json").write_text(out.name)
