...
def main(root):
    # schemas and .rtt are in parent directory
    parent_dir = os.path.dirname(root)
    schema_dir = os.path.join(parent_dir, "schemas")
    sch_sym = load(os.path.join(schema_dir, "rtt.symbol.schema.json"))
    sch_pol = load(os.path.join(schema_dir, "rtt.policy.schema.json"))
    sch_rts = load(os.path.join(schema_dir, "rtt.routes.schema.json"))
    ok = True
    for p in glob.glob(os.path.join(parent_dir, ".rtt", "manifests", "*.json")):
        obj = load(p)
        try:
    ...
    p = os.path.join(parent_dir, ".rtt", "policy.json")
    try:
        validate_schema(load(p), sch_pol, p); print(f"[OK] {p}")
    except SystemExit as e:
        print(str(e)); ok=False
    p = os.path.join(parent_dir, ".rtt", "routes.json")
    try:
