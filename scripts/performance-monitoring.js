...
class PerformanceMonitor {
    ...
    async initializePerformanceTables() {
        ...
        const existingTables = await this.allQuery("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('agent_health', 'system_alerts', 'performance_reports')", []);
    ...
    async updateAgentHealth(agentId, healthData) {
        ...
        const currentHealth = await this.allQuery(
    ...
    async getSwarmMetrics(swarmId, hours = 24) {
        ...
        const agentsInSwarm = await this.allQuery('SELECT id FROM agents WHERE swarm_id = ?', [swarmId]);
    ...
    generateRecommendations(summary, metricsSummary) {
        ...
        if (summary.criticalAlerts > 0) {
            recommendations.push(`${summary.criticalAlerts} critical alerts detected. Immediate attention required.`);
        }

        const highResponseTime = metricsSummary.find(m =>
            m.metric_name === 'response_time' && m.avg_value > 2000
        );
        if (highResponseTime) {
            recommendations.push('Average response time is high. Consider optimizing agent performance.');
        }

        const highErrorRate = metricsSummary.find(m =>
            m.metric_name === 'error_rate' && m.avg_value > 0.1
        );
        if (highErrorRate) {
        ...
        try {
            ...
            for (const agent of agents) {
                ...
                for (const [metricName, value] of Object.entries(mockMetrics)) {
                    await this.recordMetric(agent.id, 'system', metricName, value,
                        metricName.includes('time') ? 'ms' :
                        metricName.includes('usage') ? '%' : 'count');
                }

                // Update health status
                const status = mockMetrics.memory_usage > 90 || mockMetrics.cpu_usage > 80 ? 'critical' :
                              mockMetrics.memory_usage > 70 || mockMetrics.cpu_usage > 60 ? 'warning' : 'healthy';
    ...
    async getPerformanceStats() {
        const metricsCount = await this.allQuery('SELECT COUNT(*) as count FROM performance_metrics', []);
        const healthCount = await this.allQuery('SELECT COUNT(*) as count FROM agent_health', []);
        const alertsCount = await this.allQuery('SELECT COUNT(*) as count FROM system_alerts WHERE resolved = 0', []);
