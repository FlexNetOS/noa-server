...
class InterAgentCommunication {
  ...
  async initializeMessageTables() {
    ...
    const existingTables = await this.allQuery(
      "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('messages', 'message_deliveries', 'agent_subscriptions')",
      []
    );
  ...
  async subscribeToTopic(agentId, topic) {
    await this.runQuery('INSERT OR REPLACE INTO agent_subscriptions VALUES (?, ?, ?, ?, ?)', [
      null,
      agentId,
      topic,
      'topic',
  ...
  async unsubscribeFromTopic(agentId, topic) {
    await this.runQuery('DELETE FROM agent_subscriptions WHERE agent_id = ? AND topic = ?', [
      agentId,
      topic,
    ]);
  }
  ...
  async getTopicSubscribers(topic) {
    const rows = await this.allQuery('SELECT agent_id FROM agent_subscriptions WHERE topic = ?', [
      topic,
    ]);
  ...
  async getSwarmAgents(swarmId) {
    const rows = await this.allQuery('SELECT id FROM agents WHERE swarm_id = ?', [swarmId]);
