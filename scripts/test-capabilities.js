    ...
    async testAgentCapabilities() {
        ...
        const agentTests = [
            ...
            },
            {
                ...
                query: 'SELECT COUNT(*) as count FROM agents WHERE capabilities IS NOT NULL AND capabilities != "[]"',
                expected: (result) => result.count > 0,
...
${Object.entries(this.testResults.mcpTools).map(([name, result]) =>
    `- **${name}:** ${result.success ? '✅ PASSED' : '❌ FAILED'}`
).join('\n')}

## Agent Capabilities Test Results

${Object.entries(this.testResults.agentCapabilities).map(([name, result]) =>
    `- **${name}:** ${result.success ? '✅ PASSED' : '❌ FAILED'}`
).join('\n')}

## Swarm Connectivity Test Results

${Object.entries(this.testResults.swarmConnectivity).map(([name, result]) =>
    `- **${name}:** ${result.success ? '✅ PASSED' : '❌ FAILED'}`
).join('\n')}
...
*Capability testing completed*
    ...
    async run() {
        try {
            ...
            console.log(`\n🎉 Capability Testing COMPLETED!`);
            console.log(`📊 Success Rate: ${successRate.toFixed(1)}% (${this.testResults.overall.passed}/${this.testResults.overall.total} tests passed)`);
